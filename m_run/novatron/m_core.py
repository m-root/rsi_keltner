...
from payload.payload_trd import Payload, buyLogic, sellLogic
...
def buy(pair, traded_pairs):
    ...
    payload = Payload(base_asset=pair[0], cross_asset=pair[1], chat_id="-282619797", settings=settings, time_frame = settings.period)
    print('****************************************')
    print('****************************************')
    print('********** MARKER 2 *****************')
    print('****************************************')
...
def logic(pair, time_frame):
    ...
    print('\t\t\t',pair[0]+pair[1])
    print('****************************************')
    print('Buy is {}'.format(buyLogic(pair = pair,
                                      tFrame= time_frame,
                                      settings = settings
                                      ) and pair[0]+pair[1] not in buy_list))
    print()
    print('Sell is {}'.format(sellLogic(pair = pair,
                                        tFrame=time_frame,
                                        settings=settings) and pair not in sell_list))
    print('****************************************')
    if buyLogic(pair = pair,
                tFrame=time_frame,
                settings=settings) \
            and pair[0]+pair[1] not in buy_list:
        ...
        # if pair[0]+pair[1] not in buy_list and len(buy_list_list) <= (100/settings.eq_pc):
        ...
        ''' if pair[0]+pair[1] not in sell_list and len(sell_list_list) < (100/settings.eq_pc)+1: '''

        # if pair[0] + pair[1] not in sell_list and len(sell_list_list) <= (100 / settings.eq_pc):
